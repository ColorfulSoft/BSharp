...
namespace Mono.Cecil.Cil {
	...
	class InstructionCollection : Collection<Instruction> {
		...
		void UpdateDebugInformation (Instruction removedInstruction, Instruction existingInstruction)
		{
			...
			var custom_debug_infos = method.custom_infos ?? ((method.debug_info!=null) ? method.debug_info.custom_infos : null);
			if (custom_debug_infos != null) {
				foreach (var custom_debug_info in custom_debug_infos) {
			        if(custom_debug_info is StateMachineScopeDebugInformation)
			        {
			            UpdateStateMachineScope ((StateMachineScopeDebugInformation)custom_debug_info, ref resolver);
			            continue;
			        }
			        if(custom_debug_info is AsyncMethodBodyDebugInformation)
		            {
			            UpdateAsyncMethodBody((AsyncMethodBodyDebugInformation)custom_debug_info, ref resolver);
			            continue;
		            }
				}
			}
